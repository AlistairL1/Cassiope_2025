...
from maps.walls_gotta_hurry_01 import add_walls, add_boxes


class MapNarrowCorridor(MapAbstract):

    def __init__(self, zones_config: ZonesConfig = ()):
        super().__init__(zones_config)
        self._max_timestep_limit = 2000
        self._max_walltime_limit = 120  # In seconds

        # PARAMETERS MAP
        self._size_area = (1500, 200)

        # Return area (where drones start)
        self._return_area = ReturnArea(size=(100, 200))
        self._return_area_pos = ((-650, 0), 0)

        # Rescue center
        self._rescue_center = RescueCenter(size=(100, 200))
        self._rescue_center_pos = ((-730, 0), 0)

        # Wounded person at the other end of the corridor
        self._wounded_persons_pos = [(730, 0)]
        self._number_wounded_persons = len(self._wounded_persons_pos)
        self._wounded_persons: List[WoundedPerson] = []

        # POSITIONS OF THE DRONES
        self._drones_pos = [((-650, 0), 0)]
    ...
    def construct_playground(self, drone_type: Type[DroneAbstract]) -> Playground:
        ...
        # Add walls from the walls_gotta_hurry_01.py file
